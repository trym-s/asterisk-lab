...
| Softphone | Linphone (on Arch host) — see "MicroSIP deviation" |
...
Settled on libvirt's `default` NAT network. Provides the same host-only-NAT topology declaratively, ships on every distro, and the domain XML can use storage-pool volume references rather than absolute paths — so the same XML works on any host. `infra/libvrt`
...
- Use a Linux-native softphone (Linphone, Zoiper, baresip) → SIP is RFC-standard, server-side config is identical.

Chose Linphone on the Arch host for development. README will note: "developed and verified against Linphone on Linux. MicroSIP compatibility can be smoke-tested on Windows at acceptance — both speak the same SIP." This deviation is the only one from `todos.txt`.


 - There will be adapter for MicroSIP
...
auth_type=userpass
username=1001
password=LinphoneDevPass_2026!    ; `mock``
...
```
<sip:1001@192.168.122.20>;auth_pass=LinphoneDevPass_2026!;transport=udp;regint=3600;answermode=auto;audio_codecs=opus,PCMU,PCMA
```
...
| D | `extensions.conf` dialplan with `MixMonitor` recording to `/var/spool/asterisk/monitor/` | pending |
| E | Python transcription (Whisper API or local), output to a text file. Secrets via env vars. | pending |
