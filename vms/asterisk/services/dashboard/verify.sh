...
check "/api/overview returns JSON"        '"active_calls"'   "curl -sf $BASE/api/overview"
check "/api/extensions returns JSON"      '"extensions"'     "curl -sf $BASE/api/extensions"
check "/api/uptime returns JSON"          '"hosts"'          "curl -sf $BASE/api/uptime"
check "/api/calls returns JSON"           '"calls"'         "curl -sf $BASE/api/calls"
check "/api/calls detail returns JSON"    '"turns"'         "curl -sf $BASE/api/calls/__missing__/turns"
check "/api/parity returns JSON"          '"lanes"'         "curl -sf $BASE/api/parity"
check "/api/cost returns JSON"            '"rows"'          "curl -sf $BASE/api/cost"
check "/api/cost/timeseries returns JSON" '"points"'        "curl -sf $BASE/api/cost/timeseries"
check "/api/transcriber returns JSON"     '"recordings"'    "curl -sf $BASE/api/transcriber"
check "/api/comparison/fairness returns JSON"    '"rows"'   "curl -sf $BASE/api/comparison/fairness"
check "/api/comparison/quality returns JSON"     '"rows"'   "curl -sf $BASE/api/comparison/quality"
check "/api/comparison/latency returns JSON"     '"lanes"'  "curl -sf $BASE/api/comparison/latency"
check "/api/comparison/reliability returns JSON" '"comparable_outcomes"' "curl -sf $BASE/api/comparison/reliability"
check "/api/comparison/cost returns JSON"        '"rows"'   "curl -sf $BASE/api/comparison/cost"
check "/ renders 200"                     "^200$"           "curl -s -o /dev/null -w '%{http_code}' $BASE/"
check "/calls renders 200"                "^200$"           "curl -s -o /dev/null -w '%{http_code}' $BASE/calls"
check "/calls detail renders 200"         "^200$"           "curl -s -o /dev/null -w '%{http_code}' $BASE/calls/__missing__"
check "/parity renders 200"               "^200$"           "curl -s -o /dev/null -w '%{http_code}' $BASE/parity"
check "/comparison renders 200"           "^200$"           "curl -s -o /dev/null -w '%{http_code}' $BASE/comparison"
check "/cost renders 200"                 "^200$"           "curl -s -o /dev/null -w '%{http_code}' $BASE/cost"
check "/transcript redirects"             "^307$"           "curl -s -o /dev/null -w '%{http_code}' $BASE/transcript"
