...
1. Read `global/mission.md` to understand the whole lab.
2. Read `global/agent-routing.md` to select a domain.
3. Read the selected `domains/<domain>/spec.md`.
4. Read the matching `contracts/VAL-*.md` before changing behavior.
5. Read `runbook.md` before deploying or operating that domain.
6. Read `decisions.md` when a design choice looks surprising.

## Directory Model

```text
global/       Repo-wide mission, conventions, environment, validation, routing.
domains/      Current truth for each subsystem.
changes/      Change proposals and migrations. Not current truth by itself.
```

Global contracts live under `global/contracts/`. Domain contracts live under
